...
export async function POST(req: NextRequest) {
  try {
    ...
    // Generate magic link via admin API (no PKCE)
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: "Sign in to Pharo Agents",
        html: '<div style="font-family: system-ui, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;"><h1 style="font-size: 28px; font-weight: 300; font-style: italic; margin-bottom: 24px;">Pharo</h1><p style="font-size: 14px; color: #404040; line-height: 1.6; margin-bottom: 24px;">Click the button below to sign in to Pharo Agents. This link expires in 1 hour.</p><a href="' + actionLink + '" style="display: inline-block; background: #0a0a0a; color: #fff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600;">Sign in</a><p style="font-size: 12px; color: #737373; margin-top: 24px;">If you did not request this, you can safely ignore this email.</p></div>',
      }),
    });
