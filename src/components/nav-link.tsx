...
export function NavLink({ href, label, iconName }: { href: string; label: string; iconName: string }) {
  ...
  return (
    <Link href={fullHref}
      ...
      style={{
        color: isActive || hovered ? "var(--text-primary)" : "var(--text-muted)",
        background: isActive || hovered ? "var(--bg-raised)" : "transparent",
        borderLeft: isActive ? "2px solid var(--text-primary)" : "2px solid transparent",
      }}>
      <Icon size={15} strokeWidth={isActive || hovered ? 2 : 1.5} />
      {label}
    </Link>
  );
}
